...
- **The NAM translation gap** — why chip/organoid data needs a mechanistic bridge to become clinical evidence
- **Clinical pharmacology** — PBPK & QSP as that bridge
- **Our engine** — an LLM-augmented, open QSP model library (258 diseases and growing)
- **Case studies** — a dozen drug–disease vignettes and six full deep dives, incl. a live app (Merigolix)
...
- **Phase 1** trials characterize healthy-volunteer exposure well, but leave it poorly characterized in the patients who need it most: [IBD]{.chip} [AKI]{.chip} [Hepatic impairment]{.chip} [Pregnancy]{.chip} [Pediatric]{.chip} [Rare disease]{.chip}
...
| Layer | Examples | Role in the loop |
|---|---|---|
| NAM data sources | Organoid · organ-on-chip · 2D/3D culture · *in silico* | generate a context-of-use signal |
| Data science / AI-ML tools | pattern extraction across large NAM + omics datasets | connect raw readouts toward reference datasets |
| **Mechanistic models — PBPK & QSP** | dose → concentration → target → pathway → phenotype | **qualify & translate the NAM signal into clinical meaning** |
| Reference datasets | biological (protein/immune atlases) · clinical (trial endpoints) | ground truth the qualification is checked against |

- Regulatory qualification of a NAM means connecting its readout to a clinical dataset — AI/ML pattern-matching alone cannot supply that causal, dose-scaled link
- **PBPK and QSP are the mechanistic models this loop depends on** — exactly the bridge this talk is about
...
- **What a chip gives you:** a concentration–time curve · single- or multi-organ readouts · a mechanistically rich human-cell signal — genuinely human-relevant, *before* first-in-human studies
- **What a dosing decision needs:** clearance, tissue distribution, target engagement — for a **whole human body**, not a chip well
- A raw chip curve is a **descriptive** observation. It only becomes a dosing answer once it passes through a mechanistic model.
...
[The same grammar underlies both a 258-model QSP library and a chip-to-PBPK bridge.]{.fineprint}
...
Romosozumab (anti-sclerostin): a ~12-month anabolic window before resorption rebounds — timing the switch to an anti-resorptive matters.
:::
::: {.column width="33%"}
**Migraine — occupancy, not concentration**

Erenumab (anti-CGRP-receptor mAb): slow clearance sustains receptor occupancy between monthly doses, which tracks migraine-day reduction better than plasma level.
:::
::: {.column width="33%"}
**Atopic dermatitis — dual blockade**

Dupilumab (anti-IL-4Rα) silences both IL-4 and IL-13; biomarkers (IgE, TARC) move early, EASI improvement lags behind.
:::
::::
...
Emicizumab (bispecific FVIIIa-mimetic) prolongs aPTT far past any real hemostatic benefit; modeling reconciles thrombin generation with the actual bleed-rate drop.
:::
::: {.column width="33%"}
**Myasthenia gravis — clearance vs. symptom lag**

Efgartigimod (FcRn antagonist) drops IgG ~60% within days; the lag before MG-ADL improves supports cyclic, not continuous, dosing.
:::
::: {.column width="33%"}
**Gout — when the drug beats itself**

Pegloticase clears urate well at first, but anti-drug antibodies against the PEG-uricase build up and predict who loses response.
:::
::::
...
:::
::: {.column width="60%"}
- Synovial IL-6/TNF/JAK signaling → pannus formation → cartilage and bone erosion; CRP tracks IL-6 trans-signaling as an acute-phase readout
- **Drugs modeled:** methotrexate (antifolate) · tocilizumab (anti-IL-6R, TMDD) · adalimumab (anti-TNF) · baricitinib (JAK1/2 inhibitor)
- **Endpoints:** DAS28, CRP, bone erosion — mono- and combination-therapy regimens
- 19 ODEs · 7 scenarios
:::
::::
...
:::
::: {.column width="60%"}
- Dystrophin-gene mutation → absent dystrophin → sarcolemmal fragility → chronic myofiber necrosis/regeneration → fibro-fatty replacement, progressive weakness
- **Drugs modeled:** deflazacort/prednisone (corticosteroid) · eteplirsen (exon-51-skipping ASO, restores dystrophin) · givinostat (HDAC inhibitor, anti-fibrotic)
- **Endpoints:** dystrophin (% of normal), functional decline trajectory
- 22 ODEs · 6 scenarios
:::
::::
...
:::
::: {.column width="60%"}
- Amyloid-β production/clearance imbalance → plaque burden → synaptic and cholinergic loss → progressive cognitive decline; APOE4 genotype accelerates the trajectory
- **Drugs modeled:** donepezil (AChE inhibitor) · memantine (NMDA antagonist) · lecanemab (anti-amyloid mAb, plaque clearance)
- **Endpoints:** amyloid burden, MMSE-proxy cognition — genotype- and mutation-stratified scenarios
- 20 ODEs · 7 scenarios
:::
::::
...
- **Whole-body PBPK:** 10-state perfusion-limited model (+ optional permeability-limited liver); Rodgers–Rowland $K_p$; Varma extended clearance; ACAT oral absorption
...
- Apply a **relative-activity factor (RAF)** to bridge *in vitro* → *in vivo* metabolism **only if** CYP3A4 contributes ≥90%, biliary clearance <50%, and hepatic uptake isn't already captured
- A mechanism-based test, not curve-fitting — falsifiable, drug-by-drug
- Result: uniform correction → GMFE 9.7×; **selective, rule-based correction → GMFE 1.85×**
...
- **Simvastatin under cisplatin-injury chip:** AUC ratio 2.19× — hepatic-localized ($K_{puu,liver}$ halved, consistent with OATP1B1 suppression)
- The same PBPK pipeline distinguishes gut- vs. hepatic-localized shifts — hypothesis-generating for IBD/AKI-comorbid patients normally excluded from Phase 1

## Limitations, and what this means

- **No mucus/peristalsis layer** — affects chip $P_{app}$
- **HepaRG 2D CYP3A4** ~10% of *in vivo* (RAF-corrected) · **RPTEC OAT1/3** ~1% of *in vivo* (PTC-anchor corrected)
- **Sparse clinical PK in disease-state patients** — roadmap: prospective IBD/AKI cohorts
- **Open-source Python** — the full 6-drug/24-observation run reproduces in **&lt;5 minutes** on a laptop
...
::: {style="columns:2; column-gap:2em; font-size:0.62em; line-height:1.45;"}
