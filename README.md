...
| 2026-06-18 | [**Irritable Bowel Syndrome (IBS)**](#irritable-bowel-syndrome-ibs) | 만성질환 / 소화기·신경 | Brain-Gut Axis: HPA 스트레스(CRH→MC 탈과립→MC histamine/tryptase) + EC세포 5-HT 과분비(TPH1↑/LGI유발) → 5-HT3R(내장 통각 자극/운동 촉진) + 5-HT4R(연동 반사/운동 촉진) 불균형; 장내 미생물 이상(Firmicutes:Bacteroidetes 비율↑/SIBO/CH4 생성→운동 저하) → LPS-TLR4-NFκB → 비만세포 활성화 → PAR-2/TRPV1 → 말초 내장 과민→ 척수 민감화(Wind-up/NMDA-NR2B) → 중추 민감화; 장 투과성↑(claudin-2↑/ZO-1↓) → LPS 유입 → 저등급 점막 염증 지속; IBS-D(Alosetron 5-HT3R 차단, IC50=1.6 nM, Camilleri 2000 Lancet: 통증↓40%/절박↓50%) + IBS-C(Linaclotide GC-C 수용체→cGMP↑→CFTR→분비↑+통각↓, Chey 2012 NEJM: SBM+1/wk 반응률 33.7%) + Rifaximin(RNA pol β-subunit, SIBO제거; Pimentel 2011 NEJM: 전체 반응 40.8% vs 31.2%) + Loperamide(µ-OR 말초 작용) + Eluxadoline(µ+κ/δ; Lembo 2016 NEJM) + 저용량 TCA/SSRI(SERT 억제→5-HT↑+중추 신경조절); 23-CMT ODE (약물PK 8+5HT 3+운동 3+통증 4+면역 3+질환 2); 7 치료 시나리오; 스트레스-디스비오시스 민감도 분석; 알로세트론 용량-반응 분석 | [![IBS](irritable-bowel-syndrome/ibs_qsp_model.png)](irritable-bowel-syndrome/ibs_qsp_model.svg) | [R](irritable-bowel-syndrome/ibs_mrgsolve_model.R) | [refs](irritable-bowel-syndrome/ibs_references.md) | [Shiny](irritable-bowel-syndrome/ibs_shiny_app.R) |
