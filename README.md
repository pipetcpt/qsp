...
## 🫘 루푸스 신염 (Lupus Nephritis, LN) — 최신 모델 상세 (2026-06-25)

> **디렉토리:** [`lupus-nephritis/`](lupus-nephritis/) | **약어:** LN | **날짜:** 2026-06-25

[![LN QSP 기계론적 지도](lupus-nephritis/ln_qsp_model.png)](lupus-nephritis/ln_qsp_model.svg)

### 질환 개요

| 항목 | 내용 |
|------|------|
| **질환명** | 루푸스 신염 (Lupus Nephritis, LN) |
| **분류** | ISN/RPS Class I–VI (주로 III/IV/V 대상) |
| **유병률** | SLE 환자의 40–60%에서 발생 |
| **주요 기전** | IC 침착 → 보체 활성화 → 족세포 손상 → 단백뇨 |
| **핵심 바이오마커** | UPCR, eGFR, anti-dsDNA IgG, C3, C4 |
| **치료 목표** | CRR (UPCR<0.5 + eGFR≥60), PRR (UPCR<1.0 + eGFR≥60) |

### 병태생리 요약

```
Type I IFN (pDC → cGAS-STING → TLR7/9)
        ↓
BAFF ↑ → 나이브 B세포 생존 → GC 반응 → 장수 형질세포
Tfh ↑   → IL-21 → GC 반응 촉진
        ↓
Anti-dsDNA IgG ↑ + C1q → 면역복합체 (IC) 형성
        ↓
IC → 사구체 메산지움·내피세포 침착
C1q → C4 → C3 → C5a (호중구 유인) + C5b-9 (족세포 공격)
        ↓
족세포 손상 → nephrin/synaptopodin 소실 → 슬릿막 붕괴
        ↓
단백뇨 (UPCR ↑) → TGF-β → 간질 섬유화 → eGFR 감소
```
...
| 파일 | 설명 |
|------|------|
| 🗺️ 기계론적 지도 | [`ln_qsp_model.dot`](lupus-nephritis/ln_qsp_model.dot) | **100+ 노드, 15 클러스터** |
| ⚙️ mrgsolve ODE | [`ln_mrgsolve_model.R`](lupus-nephritis/ln_mrgsolve_model.R) | **20구획 ODE**, **5치료 시나리오** |
| 📊 Shiny 앱 | [`ln_shiny_app.R`](lupus-nephritis/ln_shiny_app.R) | **6탭** (환자프로파일·약물PK·면역바이오마커·신기능·임상엔드포인트·시나리오비교) |
| 📚 참고문헌 | [`ln_references.md`](lupus-nephritis/ln_references.md) | **45개 PubMed 인용** (8개 섹션) |

### 모델 클러스터 (15개)

| 클러스터 | 핵심 구성 요소 |
|---------|------------|
| ① 선천면역 | NETosis·dsDNA 방출·TLR7/9·cGAS-STING·pDC·type I IFN·DAMP |
| ② T 세포 | Th1/Th2/Th17·Tfh·Treg·IL-17A·IL-21·IFN-γ |
| ③ B 세포/자가항체 | 나이브 B→GC→형질세포·anti-dsDNA·anti-C1q·IC |
| ④ 보체계 | C1q→C4→C3→C5a→MAC·대체경로·C3 신염인자 |
| ⑤ 사구체 병리 | 메산지움 팽창·내피세포 손상·족세포 손상·단백뇨·초승달 형성 |
| ⑥ 요세관간질 | PTC·ROS·KIM-1·TGF-β1·근섬유아세포·간질 섬유화·eGFR |
| ⑦ 신장 혈역학 | RAAS·AngII·구심/원심 세동맥·사구체내 고혈압·UPCR·SLEDAI 신장 |
| ⑧ HCQ PK/PD | 분포·TLR7/9 억제·IFN-α 감소·IC 제거 증가 |
| ⑨ MMF/MPA PK/PD | IMPDH 억제·구아노신 고갈·B/T 세포 증식 억제·장간순환 |
| ⑩ Voclosporin PK/PD | 칼시뉴린 억제·NFAT 차단·IL-2↓·Th17↓·족세포 안정화 |
| ⑪ Belimumab PK/PD | BLyS/BAFF 중화·나이브/기억 B세포↓·anti-dsDNA 지연 감소 |
| ⑫ Anifrolumab PK/PD | IFNAR1 차단·ISG score↓·pDC IFN-α↓·B세포 IFN-driven 분화↓ |
| ⑬ 사이클로포스파마이드 | CYP2B6 활성화→포스파라미드머스타드→DNA 교차결합→림프구 세포자멸 |
| ⑭ 글루코코르티코이드 | GR 결합→NF-κB/AP-1 억제→IL-6·TNF-α·IFN-γ↓·부작용 |
| ⑮ 사이토카인 네트워크 | IL-6·IL-10·IL-18·IL-33·TNF-α·MCP-1·CXCL10 |

### ODE 구획 (20개)

| # | 구획 | 생물학적 의미 |
|---|------|------------|
| 1–3 | `MPA_gut`, `MPA_plasma`, `MPAG_gut` | MMF/MPA 흡수·순환·장간순환 |
| 4–5 | `HCQ_blood`, `HCQ_tissue` | HCQ 혈액/조직 2구획 |
| 6 | `VCS_plasma` | 보클로스포린 혈장 |
| 7–8 | `BEL_central`, `BEL_periph` | 벨리무맙 중심/말초 2구획 |
| 9–10 | `ANI_central`, `ANI_periph` | 아니프롤루맙 중심/말초 |
| 11–15 | `B_naive`, `B_GC`, `Plasma_cell`, `Tfh`, `Treg` | 면역세포 구획 |
| 16–18 | `Anti_dsDNA`, `C3`, `C4` | 자가항체 및 보체 |
| 19–20 | `Podocyte_inj`, `Proteinuria` | 사구체 손상 및 단백뇨 |
| 21 | `eGFR_cmt` | 신사구체 여과율 |

### 치료 시나리오 (5개)

| 시나리오 | 요법 | 임상 근거 |
|---------|------|---------|
| S1 | **표준치료: MMF 3g/day + HCQ 400mg** | ACR LN 가이드라인 2012 |
| S2 | **삼중 요법: MMF 2g + HCQ + Voclosporin 23.7mg BID** | AURORA 1 (Rovin, Lancet 2021) |
| S3 | **MMF + HCQ + Belimumab 10mg/kg q4w** | BLISS-LN (Furie, NEJM 2020) |
| S4 | **MMF + HCQ + Anifrolumab 300mg q4w (High IFN)** | TULIP-LN (Jayne, ARD 2022) |
...
## Melanoma (악성 흑색종) — 최신 모델 상세 (2026-06-25)

> **디렉토리:** [`melanoma/`](melanoma/) | **약어:** MEL | **날짜:** 2026-06-25

[![Melanoma QSP 기계론적 지도](melanoma/melanoma_qsp_model.png)](melanoma/melanoma_qsp_model.svg)

| **항목** | **내용** |
|----------|---------|
| **질환명** | 악성 흑색종 (Melanoma) |
| **주요 드라이버** | BRAF V600E/K (~50%), NRAS Q61 (~20%), NF1 손실 (~15%), WT (<10%) |
| **표적 치료** | BRAFi (베무라페닙/다브라페닙) + MEKi (트라메티닙/코비메티닙) |
| **면역 치료** | PD-1 (펨브롤리주맙/니볼루맙) + CTLA-4 (이필리무맙) |
| **5년 OS (최선 치료)** | ~52% (니볼루맙+이필리무맙, CheckMate 067) |
| **핵심 저항 기전** | BRAF 증폭, 이차 NRAS 돌연변이, MEK1/2 GoF, COT1, EGFR 상향 |

### 파일 구성

| 파일 | 설명 |
|------|------|
| 🗺️ 기계론적 지도 | [`melanoma_qsp_model.dot`](melanoma/melanoma_qsp_model.dot) | **159+ 노드, 12 클러스터** |
| ⚙️ mrgsolve ODE | [`melanoma_mrgsolve_model.R`](melanoma/melanoma_mrgsolve_model.R) | **16구획 ODE**, **6치료 시나리오** |
| 📊 Shiny 앱 | [`melanoma_shiny_app.R`](melanoma/melanoma_shiny_app.R) | **6탭** (환자 프로파일·PK·종양반응·면역동태·시나리오비교·바이오마커) |
| 📚 참고문헌 | [`melanoma_references.md`](melanoma/melanoma_references.md) | **63개 PubMed 인용** (14개 섹션) |

### 모델 클러스터 (12개)

| 클러스터 | 핵심 구성 요소 |
|---------|------------|
| ① BRAFi PK | CYP3A4/UGT1A1 대사·OATP1B·PPB >99%·Tmax 2–4h·Vd 100L |
| ② MEKi PK | CYP3A4/UGT1A3·Tmax 1.5h·t½ 3.9d (트라메티닙)·Vd 214L |
| ③ ICI PK | 2구획 IV PK·TMDD·FcRn 재활용·t½ 25–27d·PD-1/CTLA-4 수용체 점유율 |
| ④ MAPK 신호 | RAS→BRAF V600E/K(구성적)→MEK1/2→ERK1/2→핵 전위·DUSP6 음성 피드백·CRAF 역설 활성화 |
| ⑤ 전사인자·세포주기 | MITF·MYC·FOS/JUN·사이클린D1/E1·CDK4/6·RB1·CDKN2A p16·p53·MDM2 |
| ⑥ PI3K/AKT/mTOR | PTEN 손실→PIP3↑→AKT→mTORC1/S6K·내성 우회 경로 |
| ⑦ 내인성 세포자멸 | BCL-2/xL/MCL1 vs BIM/PUMA/NOXA→BAX/BAK→MOMP→caspase 폭포 |
| ⑧ 항원 제시·T세포 프라이밍 | MHC-I/II·프로테아좀·TAP·CD28/B7·CD40·IFN-γ→PD-L1↑·성숙 DC |
| ⑨ T세포 생물학 | CD8+ TIL 모집·소진(TIM-3/LAG-3/TIGIT)·Granzyme B/Perforin·IL-2·IFN-γ·Treg |
| ⑩ 종양 미세환경 | VEGF/혈관신생·TGF-β·IL-6/10·MDSC·TAM(M2)·EMT·저산소·HIF-1α |
| ⑪ 획득 내성 | BRAF 증폭/스플라이스 변이·이차 NRAS Q61·MEK1/2 GoF·COT1·DUSP6 손실·ERK 재활성화 |
| ⑫ 임상 엔드포인트 | ORR/DCR/PFS/OS·반응 깊이·LDH·S100B·ctDNA·TMB·MSI-H |

### 치료 시나리오 (6개)

| 시나리오 | 요법 | 임상 근거 |
|---------|------|---------|
| S1 | **BRAF V600E 무치료** | 역사적 중앙 OS ~8개월 |
| S2 | **베무라페닙 960mg BID** | BRIM-3 (NEJM 2011) — ORR 48%, mPFS 5.3mo |
| S3 | **다브라페닙 150mg BID + 트라메티닙 2mg QD** | COMBI-d (NEJM 2014) — ORR 67%, mPFS 9.3mo, 5yr OS 28% |
| S4 | **펨브롤리주맙 200mg q3w** | KEYNOTE-006 (NEJM 2015) — ORR 33%, mPFS 5.6mo, 3yr OS 50% |
| S5 | **니볼루맙 1mg/kg + 이필리무맙 3mg/kg q3w×4 → 니볼루맙 유지** | CheckMate 067 — ORR 58%, mPFS 11.5mo, 5yr OS 52% |
...
## 170. 근긴장성 이영양증 제1형 (Myotonic Dystrophy Type 1, DM1)

> **디렉토리:** `myotonic-dystrophy/` | **약어:** DM1 | **날짜:** 2026-06-26  
> **분류:** 신경근육 (Neuromuscular) | **OMIM:** #160900

[![DM1 QSP Mechanistic Map](myotonic-dystrophy/dm1_qsp_model.png)](myotonic-dystrophy/dm1_qsp_model.svg)
        ...
        └─► Mutant DMPK mRNA (CUGn) retained in nucleus
              └─► CUGn RNA Hairpin → Nuclear RNA Foci
                    ...
                    └─► PKCβII Activation → CUGBP1/CELF1 Hyperphosphorylation (gain-of-function)
                          ...
                          ├─► INSR fetal isoform  → Insulin resistance (T2DM ×3)
                          ...
                          └─► CAMK2D/KCNQ1 splicing → Arrhythmia risk (SCD 30%)
```

### QSP 모델 구성

- **기계론적 지도**: ~158 노드 · 10 서브그래프 클러스터 (①유전·분자 ②RNA결합단백질 ③이상 선택적 스플라이싱 ④골격근 ⑤심장 ⑥CNS·인지 ⑦내분비·대사 ⑧약동학 ⑨약력학 ⑩임상 엔드포인트)
- **mrgsolve ODE**: 22구획 — 멕실레틴 3구획(GUT·CENT·PERI) + ASO 3구획(PLASMA·MUSCLE·NUCL) + 질환 생물학 6구획(CUG_FOCI·MBNL1_FREE·CUGBP1_ACT·CLCN1_FETAL·SERCA_FETAL·INSR_FETAL) + 임상 엔드포인트 7구획(MYOTONIA·GRIP_STR·MUSCLE_MASS·PR_INT·QTc_INT·HOMA_IR·FVC_PCT)
- **치료 시나리오 7가지**: ①자연 경과 ②멕실레틴 200mg TID ③멕실레틴 300mg TID(MELT 용량) ④ASO 4주 간격(DYNE-101) ⑤멕실레틴+ASO 복합 ⑥AAV-MBNL1 유전자치료(실험) ⑦중증 DM1(CTG=1200)
- **Shiny 대시보드**: 7탭 — 환자 프로파일·약물 PK·근육·근긴장·심장 안전성·시나리오 비교·바이오마커 패널·CNS·대사
...
| 파일 | 설명 |
|------|------|
| [`dm1_qsp_model.dot`](myotonic-dystrophy/dm1_qsp_model.dot) | Graphviz 기계론적 지도 (~158 노드, 10 클러스터) |
| [`dm1_qsp_model.svg`](myotonic-dystrophy/dm1_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`dm1_qsp_model.png`](myotonic-dystrophy/dm1_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
...
## 172. 만성 비부비동염 동반 비용종 (Chronic Rhinosinusitis with Nasal Polyps, CRSwNP)

> **디렉토리:** [`chronic-rhinosinusitis-with-nasal-polyps/`](chronic-rhinosinusitis-with-nasal-polyps/) | **약어:** CRSwNP | **날짜:** 2026-06-26  
> **분류:** 호흡기·이비인후과 (Respiratory/ENT) | **ICD-10:** J33 (비용종) / J32 (부비동염) | **관련 endotype:** Type 2 eosinophilic (≥90% Western), non-eosinophilic (Asian)  
> **유병률:** 전체 인구의 약 4–5% (성인); 중증 CRSwNP ~1–2%

[![CRSwNP QSP 기계론적 지도](chronic-rhinosinusitis-with-nasal-polyps/crsnp_qsp_model.png)](chronic-rhinosinusitis-with-nasal-polyps/crsnp_qsp_model.svg)

### 병태생리 핵심 경로

```
상피세포 손상 (알레르겐·병원체·오염물질·SEB 등)
        │
        ▼  Alarmin 방출
   TSLP + IL-33 + IL-25 (type 2 경보소)
        │
   ┌────┼────────────────────────────────────┐
   ▼   ▼                                    ▼
 ILC2 MastCell               DCs (Th2-polarizing)
   │    │                        │
   │    │ → Histamine, PGD2,     │ → Th2 polarization
   │    │   LTC4/D4, Tryptase    │
   ▼                             ▼
IL-4, IL-5, IL-13 ─────────────────► IL-4, IL-5, IL-13
   │
   ├─ IL-4/IL-13 → IL-4Rα → IgE class switching → Plasma cell IgE ↑
   ├─ IL-4/IL-13 → Goblet cell hyperplasia → MUC5AC↑, Rhinorrhea
   ├─ IL-13 → Periostin (POSTN) → Subepithelial fibrosis
   ├─ IL-13 → Eotaxin-3 (CCL26) → Tissue eosinophil recruitment
   └─ IL-5 → Blood Eosinophil ↑ → Tissue Eos → ECP/MBP/EPX degranulation
                                                      │
                                        TGF-β1 ──────►│───► Fibrosis
                                        VEGF ─────────►│───► Angiogenesis → Edema
                                                        ▼
                                              NASAL POLYP GROWTH (NPS ↑)
```

### QSP 모델 구성

- **기계론적 지도**: **154+ 노드 · 11 서브그래프 클러스터** (①상피장벽·경보소 ②선천면역 ILC2·비만세포·호염기구·DC ③적응면역 Th2·Treg·Tfh·형질세포 ④사이토카인·케모카인·지질매개물 ⑤호산구 생물학 ⑥IgE 축·FcεRI ⑦조직 리모델링·비용종 성장 ⑧약물 PK/PD ⑨신경성 염증·감각 증상 ⑩임상 엔드포인트·바이오마커 ⑪내표형·동반질환)
- **mrgsolve ODE**: **22구획** — dupilumab 2-구획 SC + 4종 바이오로직 각 1구획 SC PK 11구획; EPI·TSLP·ILC2·TH2·IL4·IL5·IL13·IGE·EOSB·EOST·GOBC·TGFB·VEGF·NPS 14 disease 구획 (OBS_VAS·SNOT22·OLFACT·BLD_EOS·FeNO·PERIOSTIN 유도 변수)
- **치료 시나리오 7가지**: ①무치료 ②INCS 단독 ③Dupilumab 300mg q2w+INCS ④Mepolizumab 100mg q4w+INCS ⑤Benralizumab 30mg q4w→q8w+INCS ⑥Omalizumab 300mg q4w+INCS ⑦Tezepelumab 210mg q4w+INCS
- **Shiny 대시보드**: 7탭 — 환자 프로파일·바이오마커 선택 / 약물 PK / 사이토카인·바이오마커 동태 / 임상 엔드포인트 (NPS·SNOT-22·PNIF·OLFACT) / 시나리오 비교 / 반응자 분석 / 장기 치료 중단 후 재발 시뮬레이션

### 주요 임상시험 보정

| 임상시험 | 치료 | 1차 결과 | 참고문헌 |
|---------|------|---------|---------|
| **LIBERTY NP SINUS-24** | Dupilumab 300mg q2w | ΔNPS −1.8 (vs. −0.2 위약) | *Lancet* 2019 |
| **LIBERTY NP SINUS-52** | Dupilumab 300mg q2w | ΔNPS −2.1; ΔSNOT-22 −19.7 | *Lancet* 2019 |
| **SYNAPSE** | Mepolizumab 100mg q4w | ΔNPS −0.9; 혈중 Eos −55% | *Lancet Respir Med* 2021 |
| **OSTRO** | Benralizumab 30mg q4-8w | ΔNPS −0.9; 혈중 Eos >90% ↓ | *J Allergy Clin Immunol* 2022 |
| **POLYP 1&2** | Omalizumab (wt/IgE 기반) | ΔNPS −1.1 | *NEJM / Lancet Respir Med* |
| **WAYPOINT** | Tezepelumab 210mg q4w | ΔNPS −1.3; 혈중 Eos −66% | *Lancet Respir Med* 2023 |

### 바이오마커 기반 약물 선택 가이드

| 바이오마커 | 임계값 | 권장 약물 | 근거 |
|-----------|--------|---------|------|
| 혈중 호산구 | ≥300 cells/μL | Benralizumab, Mepolizumab | SYNAPSE, OSTRO 하위그룹 |
| 혈중 호산구 | ≥150 cells/μL | Dupilumab, Tezepelumab | 모든 내표형에서 유효 |
| 총 IgE | 30–1,500 kU/L + 체중기준 | Omalizumab | POLYP 1&2 적응증 |
| 퍼리오스틴 | ≥25 ng/mL | Dupilumab (IL-13 fibrotic) | IL-13-driven 표현형 |
| TSLP 과발현 | 조직 생검 | Tezepelumab | 가장 상류 차단 |
| 모든 내표형 | — | Tezepelumab, Dupilumab | 내표형 무관 효과 |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`crsnp_qsp_model.dot`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_qsp_model.dot) | Graphviz 기계론적 지도 (154+ 노드, 11 클러스터) |
| [`crsnp_qsp_model.svg`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`crsnp_qsp_model.png`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`crsnp_mrgsolve_model.R`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (22구획, 7 시나리오) |
| [`crsnp_shiny_app.R`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_shiny_app.R) | Shiny 인터랙티브 대시보드 (7탭) |
| [`crsnp_references.md`](chronic-rhinosinusitis-with-nasal-polyps/crsnp_references.md) | 참고문헌 60편 (18개 섹션) |
| [`README.md`](chronic-rhinosinusitis-with-nasal-polyps/README.md) | 디렉토리 상세 문서 |

---

## 173. 알코올성 간질환 (Alcoholic Liver Disease, ALD)

> **디렉토리:** [`alcoholic-liver-disease/`](alcoholic-liver-disease/) | **약어:** ALD | **날짜:** 2026-06-26  
> **유병률:** 전 세계 알코올 기여 사망 ~300만/년(전체 사망의 5.3%); 고소득국 간질환 사망 원인 1위  

[![ALD QSP Mechanistic Map](alcoholic-liver-disease/ald_qsp_model.png)](alcoholic-liver-disease/ald_qsp_model.svg)
...
```
에탄올 섭취
  ├─ ADH1B/1C → 아세트알데히드 (독성) + NADH/NAD+↑
  ├─ CYP2E1 (만성 유도) → 아세트알데히드 + 초과산화물(O₂•⁻) → H₂O₂ → •OH
  │    └─ 지질 과산화 → 4-HNE·MDA → 단백질·DNA 부가물
  ├─ GSH 고갈 → 페롭토시스(GPX4↓) / Nrf2/KEAP1/ARE 방어 활성화
  │
  ├─ 장 미생물 불균형 → 장 투과성↑(ZO-1·Occludin↓)
  │    └─ LPS 문맥 유입 → TLR4/MD-2/CD14 → MyD88/TRIF
  │         └─ Kupffer 세포 NF-κB → NLRP3/ASC/Casp-1 → IL-1β + 가스더민D(피롭토시스)
  │              └─ TNF-α·IL-6·CXCL8 → 호중구 CXCR2 동원 → NETs·MPO·HOCl → 간세포 괴사
  │
  └─ 간세포 손상 경로
       ├─ 아포토시스: FasL/TNFR1 → Casp-8 → Casp-3/7
       ├─ 괴사사멸: RIPK3/MLKL → 막 파열
       └─ ER 스트레스: GRP78·PERK/CHOP → BAX/BAK → Cyto-c → Apoptosome
  
  ALT/빌리루빈/INR 상승 → MELD·Maddrey DF·Lille 점수 → 90일 사망률
  TGF-β1/SMAD2-3 + PDGF-BB → 간성상세포(HSC) 활성화 → 콜라겐 I/III 침착 → 간경변(F4)
```

### mrgsolve ODE 모델 (22구획)

- **PD 구획 (14개)**: ETOH·AA·ROS·GSH·LPS·KC·TNF·IL1B·NEUT·H(간세포 생존율)·ALT·BILI·INR·F(섬유화)
- **PK 구획 (8개)**: 프레드니솔론 3구획(장·중심·말초)·NAC·G-CSF·펜톡시필린·아나킨라
...
| S1 | 지속 음주 (무치료) | 자연 경과 |
| S2 | 금주 단독 | — |
| S3 | 프레드니솔론 40mg QD × 28일 | STOPAH (Thursz 2015 *NEJM*) |
| S4 | N-아세틸시스테인 IV (GET 프로토콜) | Nguyen-Khac 2011 *NEJM* |
| S5 | 프레드니솔론 + NAC 병용 | GET 프로토콜 |
| S6 | G-CSF 5μg/kg SC × 5일 | Spahr 2008 *Hepatology* |
| S7 | 프레드니솔론 + 아나킨라 (IL-1Ra) | Pose 2021 *JHEP Rep* |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`ald_qsp_model.dot`](alcoholic-liver-disease/ald_qsp_model.dot) | Graphviz 기계론적 지도 (200+ 노드, 10 클러스터) |
| [`ald_qsp_model.svg`](alcoholic-liver-disease/ald_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`ald_qsp_model.png`](alcoholic-liver-disease/ald_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`ald_mrgsolve_model.R`](alcoholic-liver-disease/ald_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (22구획, 7 시나리오) |
| [`ald_shiny_app.R`](alcoholic-liver-disease/ald_shiny_app.R) | Shiny 인터랙티브 대시보드 (7탭) |
| [`ald_references.md`](alcoholic-liver-disease/ald_references.md) | 참고문헌 61편 (14개 섹션) |
| [`README.md`](alcoholic-liver-disease/README.md) | 디렉토리 상세 문서 |

---

## 174. 만성 혈전색전성 폐동맥 고혈압 (Chronic Thromboembolic Pulmonary Hypertension, CTEPH)

> **디렉토리:** [`chronic-thromboembolic-pulmonary-hypertension/`](chronic-thromboembolic-pulmonary-hypertension/) | **약어:** CTEPH | **날짜:** 2026-06-26  
> **유병률:** 급성 폐색전증 생존자의 ~3–5%에서 발생; 유병률 ~3–30명/백만(진단 저평가 가능성 높음)

[![CTEPH QSP Mechanistic Map](chronic-thromboembolic-pulmonary-hypertension/cteph_qsp_model.png)](chronic-thromboembolic-pulmonary-hypertension/cteph_qsp_model.svg)
...
```
급성 폐색전증
  ├─ 불완전 혈전 분해 (tPA↓·PAI-1↑·비정상 피브린 가교)
  │    └─ 조직화 혈전 → 신생내막 형성 → 기계적 고정 PVR↑ (PVR_fixed)
  │
  ├─ 이차 폐혈관 리모델링
  │    ├─ 내피세포 기능 부전 → ET-1↑ · NO↓ · PGI₂↓
  │    ├─ ET-1 → SMC 증식·이주 → 내막 비후·중막 비대
  │    └─ TGF-β/PDGF → 외막 섬유화 → 혈관 PVR 동적 성분↑ (PVR_var)
  │
  ├─ 총 PVR↑ → mPAP↑ → 우심실 압력 과부하
  │    ├─ 적응 단계: RV 비후 (compensated)
  │    └─ 부전 단계: RV 확장·수축 장애·삼첨판 역류·심박출량↓
  │
  └─ CO↓ + V/Q 불일치 → SaO₂↓ → BNP↑ → 6MWD↓ → WHO FC 악화
```

### mrgsolve ODE 모델 (19구획)

- **약물 PK (7개)**: 리오시과트 C/P/M1 대사체·마시텐탄 C/P/ACT-132577·트레프로스티닐 C
- **질환 PD (12개)**: TB·PVR_fixed·PVR_var·ET1·cGMP·cAMP·RV_work·mPAP·CO·SaO₂·BNP·6MWD

### 치료 시나리오 7가지

| # | 요법 | 예상 6MWD 변화 | 근거 임상시험 |
|---|------|---------------|--------------|
| S1 | 무치료 | −20 m/yr 진행 | 자연 경과 |
| S2 | 항응고 단독 | 안정 | — |
| S3 | 리오시과트 2.5mg TID | +46 m (wk16) | CHEST-1 (Ghofrani 2013 *NEJM*) |
| S4 | 마시텐탄 10mg QD | +35–40 m | SERAPHIN (Pulido 2013 *NEJM*) |
| S5 | 리오시과트 + 마시텐탄 | +60–75 m | MERIT-1 (Meyer 2022 *ERJ*) |
| S6 | BPA 5회 + 리오시과트 | +70–90 m | Wiedenroth 2020 |
| S7 | PEA 수술 후 + 병용 | +120–150 m | Madani 2012 *ATS* |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`cteph_qsp_model.dot`](chronic-thromboembolic-pulmonary-hypertension/cteph_qsp_model.dot) | Graphviz 기계론적 지도 (134+ 노드, 12 클러스터) |
| [`cteph_qsp_model.svg`](chronic-thromboembolic-pulmonary-hypertension/cteph_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`cteph_qsp_model.png`](chronic-thromboembolic-pulmonary-hypertension/cteph_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`cteph_mrgsolve_model.R`](chronic-thromboembolic-pulmonary-hypertension/cteph_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (19구획, 7 시나리오) |
...
## 176. 간질성 방광염 / 방광 통증 증후군 (Interstitial Cystitis / Bladder Pain Syndrome, IC/BPS)

> **디렉토리:** [`interstitial-cystitis/`](interstitial-cystitis/) | **약어:** IC/BPS | **날짜:** 2026-06-26  
> **유병률:** 여성 2–7%, 남성 0.5%에서 발생; 미국 300–800만 명 추정; 만성 골반통의 주요 원인

[![IC/BPS QSP Mechanistic Map](interstitial-cystitis/ic_bps_qsp_model.png)](interstitial-cystitis/ic_bps_qsp_model.svg)
...
트리거 (스트레스 · 감염 · 자가면역 · 유전 · 알레르겐)
  │
  ▼
GAG층 결핍 (헤파란황산·콘드로이틴황산·히알루론산 소실)
  │  → 요로상피 투과성↑ → 요중 K+ 누출 → C-섬유 탈분극
  │
  ├─ 비만세포 활성화 → 히스타민·트립타아제·PGE2·TNF-α·SP 분비
  │    └─ PAR2 활성화 → 요로상피 장벽↓ (양성 피드백)
  │
  ├─ C-섬유 (TRPV1+, P2X3+) 감작
  │    └─ SP·CGRP 역행성 분비 → 신경성 염증 → 비만세포 추가 자극
  │
  ├─ NGF ↑ → TrkA → C-섬유 증식 + TRPV1 상향조절
  │
  ├─ 척수 감작 (Wind-up · NMDA · BDNF)
  │    └─ 중추 감작 (ACC · 도서엽 · 전전두엽) → 통증 증폭·무감각화
  │
  └─ 방광벽 리모델링 (TGF-β1 → 섬유화 → 방광 용량↓ → 빈뇨)

VAS 통증↑ · O'Leary-Sant 점수↑ · 방광 용량↓ · 배뇨 빈도↑
```

### mrgsolve ODE 모델 (22구획)

- **약물 PK (8개)**: PPS 2구획 · 히드록시진 2구획 · CsA 2구획 · 아미트립틸린 2구획
- **질환 PD (14개)**: GAG · PERM · MC · HIST · SP · NGF · IL6 · TNF · C_FIBER · SPINAL · CENTRAL · CAP · PAIN · OLS
...
|---|------|----------|--------------|
| S1 | 무치료 | 자연 경과 | — |
| S2 | PPS (Elmiron) 100mg TID | GAG층 복원 | Nickel 2005 *Urology*; Hanno 2003 |
| S3 | 히드록시진 25mg QD | H1 차단·비만세포 안정화 | Sant 2003 *J Urol* |
| S4 | 방광내 DMSO 50% q2주×6회 | 항염증·비만세포 억제 | Sant 1987 *Urology* |
| S5 | 시클로스포린 A 3mg/kg/day | T세포 억제 (Hunner형) | Sairanen 2005 *J Urol* |
| S6 | BoNTA 100U 방광내 주입 | SP·CGRP 분비 억제 | Gottsch 2011 *J Urol* |
| S7 | 삼중 병용 (PPS + HYD + AMI) | 다중 기전 복합 | Foster 2010 *J Urol* |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`ic_bps_qsp_model.dot`](interstitial-cystitis/ic_bps_qsp_model.dot) | Graphviz 기계론적 지도 (140+ 노드, 9 클러스터) |
| [`ic_bps_qsp_model.svg`](interstitial-cystitis/ic_bps_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`ic_bps_qsp_model.png`](interstitial-cystitis/ic_bps_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`ic_bps_mrgsolve_model.R`](interstitial-cystitis/ic_bps_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (22구획, 7 시나리오, VP n=200) |
...
## 178. 교모세포종 (Glioblastoma Multiforme, GBM)

> **디렉토리:** [`glioblastoma/`](glioblastoma/) | **약어:** GBM | **날짜:** 2026-06-26  
> **유병률:** 성인 원발성 뇌종양의 ~50%; 미국 연간 ~13,000명; 중앙 생존기간 15–22개월 (MGMT 메틸화 여부에 따라)

[![GBM QSP Mechanistic Map](glioblastoma/gbm_qsp_model.png)](glioblastoma/gbm_qsp_model.svg)
...
```
유전자 이상 (EGFR 증폭·EGFRvIII·PTEN 소실·IDH 야생형)
  │
  ▼
RTK/RAS-MAPK ── PI3K/AKT/mTOR ── 세포주기(CDK4/6·RB1·p53) 과활성화
  │
  ├─ DNA 손상 (TMZ → O6-MeG)
  │    ├─ MGMT 메틸화: 복구 불가 → 세포사멸 ← TMZ 반응성
  │    └─ MGMT 비메틸화: 신속 복구 → TMZ 내성
  │
  ├─ HIF-1α → VEGF-A → 혈관신생 → 종양 성장 지원
  │    └─ 베바시주맙 (anti-VEGF) → VEGF-A 차단
  │
  ├─ M2-TAM · Treg · PD-L1 → 면역억제 미세환경
  │    └─ 항-PD-1 (펨브롤리주맙) → PD-1 점유 → T세포 활성화
  │
  └─ 교모세포 줄기세포 (GSC) → 방사선·TMZ 내성 + 재발원

임상 엔드포인트: RANO 직경↓ · OS · PFS · MGMT 상태 · CD8/Treg 비
```

### mrgsolve ODE 모델 (18구획)

- **TMZ PK (5구획)**: 경구 흡수(Gut) · 혈장 2구획(Cp·Cp2) · 뇌 구획(Cbrain·Kp,brain=0.28) · DNA 병변(O6MeG)
- **베바시주맙 PK (4구획)**: 혈장 2구획(BEV_Cp·BEV_Cp2) · 유리 VEGF(VEGF_free) · BEV-VEGF 복합체
- **항-PD-1 (2구획)**: 혈장(APD1_Cp) · 수용체 점유율(PD1_occ)
- **종양 PD (3구획)**: 민감 세포(Ts) · 내성 세포(Tr) · 교모세포 줄기세포(GSC)
- **면역 PD (3구획)**: CD8+ T세포(CD8_eff) · 조절 T세포(Treg_c) · M2-TAM(TAM_M2)
- **혈관신생 (1구획)**: 신생혈관 밀도(NV)
...
|---|------|----------|--------------|
| S1 | 무치료 | Gompertz 자연 경과 | — |
| S2 | Stupp + MGMT 메틸화 | TMZ O6-MeG·RT LQ | Stupp 2005 NEJM / Hegi 2005 NEJM |
| S3 | Stupp + MGMT 비메틸화 | kMGMT 높음 → 내성 | Stupp 2005 NEJM |
| S4 | Stupp + 베바시주맙 | VEGF 차단 → 혈관 정상화 | AVAGLIO·RTOG0825 2014 |
| S5 | Stupp + TTF (200kHz) | 유사분열 방해 | EF-14 (Stupp 2017 JAMA) |
| S6 | 펨브롤리주맙 + TMZ (재발) | PD-1 점유·면역 활성 | Keynote-028 / Reardon 2020 |
| S7 | 베바시주맙 단독 구제 | VEGF·PFS 연장 | Friedman 2009 JCO |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`gbm_qsp_model.dot`](glioblastoma/gbm_qsp_model.dot) | Graphviz 기계론적 지도 (220+ 노드, 13 클러스터) |
| [`gbm_qsp_model.svg`](glioblastoma/gbm_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`gbm_qsp_model.png`](glioblastoma/gbm_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`gbm_mrgsolve_model.R`](glioblastoma/gbm_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (18구획, 7 시나리오) |
| [`gbm_shiny_app.R`](glioblastoma/gbm_shiny_app.R) | Shiny 인터랙티브 대시보드 (7탭) |
| [`gbm_references.md`](glioblastoma/gbm_references.md) | 참고문헌 60편+ (13개 섹션) |
| [`README.md`](glioblastoma/README.md) | 디렉토리 상세 문서 |

---

## 179. 포도막염 (Uveitis)

> **디렉토리:** [`uveitis/`](uveitis/) | **약어:** UVT | **날짜:** 2026-06-26

[![UVT QSP Mechanistic Map](uveitis/uvt_qsp_model.png)](uveitis/uvt_qsp_model.svg)
...
```
유발 기전 (HLA-B27·IRBP/S-Ag 자가항원·감염성 트리거·VKH·Birdshot)
  │
  ▼
APC 활성화 (DC/대식세포) → TCR/CD28 → T세포 분화
  │
  ├─ Th1 (IFN-γ) ──────→ M1 대식세포 활성화 → TNF-α↑
  ├─ Th17 (IL-17A) ─────→ 중성구 동원 → 조직 손상
  └─ Treg (FoxP3+) ─────→ 조절 (감소 시 병리 심화)
       │
  TNF-α · IL-6 · IL-1β · VEGF-A 분비
       │
  ├─ 혈-안방장벽(BAB) 파괴 → 전방세포 유입(SUN 등급↑) → 플레어↑
  │    └─ 이차녹내장 / 후유착
  └─ 혈-망막장벽(BRB) 파괴 (VEGF 주도) → CME (CST↑) → 시력↓
       └─ 만성화 → 신생혈관·망막박리·실명
```

### mrgsolve ODE 모델 (20구획)

- **Drug PK (6구획)**: Cgut(경구 흡수)·Cp(혈장 1구획)·Cperiph(말초)·C_ant(전방방)·C_post(후방)·C_depot(IVT 서방 임플란트)
- **면역 (4구획)**: T_eff(Th1/Th17), T_reg(Treg), APC_act(DC/수지상세포), Macro(M1 대식세포)
- **사이토카인 (4구획)**: TNF, IL6, IL17, VEGF
- **장벽 (2구획)**: BAB_int(혈-안방 무결성 0-1), BRB_int(혈-망막 무결성 0-1)
- **임상 PD (4구획)**: Cells_AH(전방세포), CME(낭포황반부종), VA_def(시력결손 logMAR), IOP_e(IOP 과잉)
- **GR 점유 (1구획)**: GR_occ(글루코코르티코이드 수용체 점유율)
...
|---|------|----------|--------------|
| S1 | 무치료 | 자연 경과 | — |
| S2 | 점안 프레드니솔론 1% QID | 국소 GR 활성화·NF-κB 억제 | Cunningham 1990 *Ophthalmology* |
| S3 | 안와주위 트리암시놀론 40mg | 지속성 스테로이드 | Sallam 2011 *Curr Opin Ophthalmol* |
| S4 | IVT 덱사메타손 임플란트 (Ozurdex) | 후방 BRB 회복·CME 65% 해소 | HURON Trial (Lowder 2011 *Arch Ophthalmol*) |
| S5 | 전신 프레드니손 1mg/kg/일 (점감) | 전신 면역억제·Treg ↑ | MUST Trial (Kempen 2011 *Ophthalmology*) |
| S6 | 아달리무맙 40mg SC q2w | TNF-α 중화·재발 HR 0.50 | VISUAL I (Jaffe 2016 *NEJM*) |
| S7 | 병합 (프레드니손 + 아달리무맙) | 이중 기전·빠른 장벽 회복 | VISUAL I/II |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`uvt_qsp_model.dot`](uveitis/uvt_qsp_model.dot) | Graphviz 기계론적 지도 (130+ 노드, 12 클러스터) |
| [`uvt_qsp_model.svg`](uveitis/uvt_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`uvt_qsp_model.png`](uveitis/uvt_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`uvt_mrgsolve_model.R`](uveitis/uvt_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (20구획, 7 시나리오, VP n=200) |
