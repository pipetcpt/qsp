...
| 178 | 안과 | [**녹내장**<br><sub>Glaucoma · GLAUC</sub>](glaucoma/) | <a href="glaucoma/glauc_qsp_model.svg"><img src="glaucoma/glauc_qsp_model.png" width="190" alt="GLAUC"></a> | **안압(IOP) 상승 → 시신경유두(ONH) 구조 손상 → 망막신경절세포(RGC) 진행성 소실 → 시야 결손**. Goldman 방정식(`IOP=(F−Fu)/C+Pv`) 기반 방수역학 완전 모델: 섬모체 β2-AR/CA-II 억제 → 방수 생성 감소; ROCK1/2 → 섬유주 이완 → 유출 증가; FP수용체 → ECM 리모델링 → 포도막-공막 유출 증가. RGC 소실: `dRGC/dt=−kRGC×(IOP_excess²+0.3×OPP_deficit)×RGC` → RNFL 두께(OCT) 90일 지연 추적 → 시야 MD(dB) 구조-기능 관계 모델(20% RNFL 손실 임계). **147+ 노드 12 클러스터**(①위험인자 ②약물PK ③분자표적·신호 ④방수역학 ⑤안압동태 ⑥시신경유두 ⑦RGC병리 ⑧신경보호 ⑨임상엔드포인트 ⑩녹내장유형 ⑪약물PD ⑫시각경로), **26구획 ODE**(PK 10: LAT·TIM·DZL·BRI·NET 점안+전방챔버; 신호 5: FP_RO·β-AR·α2-AR·CA_INH·ROCK_INH; 방수 5: ECM·cAMP·AQ_PROD·C_TRAB·Fu_UVEA; 질환 6: IOP·ONH·RGC·RNFL·VF_MD·OPP), **7치료 시나리오**(①무치료 ②라타노프로스트 QD ③티몰롤 BID ④라타노+티몰롤 ⑤3제병용 ⑥네타수딜 QD ⑦네타수딜+라타노 Rocklatan®). OHTS(Gordon 2002)·EMGT(Heijl 2002)·LiGHT(Gazzard 2019)·CIGTS·AGIS·ROCKET-1/2(Serle 2018) 보정. IOP(mmHg)·RNFL 두께(µm)·VF MD(dB)·RGC 생존율(%)·OPP·ONH 응력 엔드포인트. **8탭 Shiny 대시보드**(환자프로파일&OHTS 위험계산기·약물PK&수용체점유·방수역학&IOP·RGC&신경보호·임상엔드포인트·7시나리오비교·바이오마커패널·민감도분석). **62편 PubMed 인용** (15개 섹션).<br>[🗺️ 지도](glaucoma/glauc_qsp_model.svg) · [⚙️ mrgsolve](glaucoma/glauc_mrgsolve_model.R) · [📊 Shiny](glaucoma/glauc_shiny_app.R) · [📚 문헌](glaucoma/glauc_references.md) · [📄 README](glaucoma/README.md) |
...
## 👁️ 녹내장 (Glaucoma) — 최신 모델 상세 (2026-06-26)

> **디렉토리:** [`glaucoma/`](glaucoma/) | **약어:** GLAUC | **날짜:** 2026-06-26

[![GLAUC QSP 기계론적 지도](glaucoma/glauc_qsp_model.png)](glaucoma/glauc_qsp_model.svg)
...
안압(IOP) 상승 원인
  ├─ 방수 생성 증가
  │    └─ 섬모체 CA-II / β2-AR / cAMP-PKA 신호 → Na/K-ATPase → HCO₃⁻ 공동수송 → F↑
  │
  ├─ 섬유주 유출 감소
  │    └─ ROCK1/2 → 액틴 세포골격·Rho → 섬유주 강도↑ → C↓
  │
  └─ 포도막-공막 유출 감소
       └─ ECM (콜라겐·MMP 불균형) → 방수 유출 저항↑ → Fu↓

Goldman 방정식: IOP = (F − Fu) / C + EVP

IOP 상승 → ONH 사상판(LC) 변형·기계적 압박
  ├─ 축삭 수송 차단 → BDNF 결핍
  ├─ 미토콘드리아 기능이상 → 산화 스트레스
  └─ 소교세포·성상교세포 활성화 → 신경염증

RGC 세포사멸 (dRGC/dt = −kRGC × (IOP_excess² + 0.3 × OPP_deficit) × RGC)
  └─ RNFL 두께 감소 (OCT, τ=90일)
       └─ 시야 MD 악화 (>20% RNFL 손실 임계)

임상 엔드포인트: IOP(mmHg) · RNFL(µm) · VF MD(dB) · RGC 생존율(%) · OPP
```

### mrgsolve ODE 모델 (26구획)

- **약물 PK (10 구획)**:
  - 점안 흡수조: LAT_DEPOT · TIM_DEPOT · DZL_DEPOT · BRI_DEPOT · NET_DEPOT
  - 전방 챔버 농도: LAT_AC · TIM_AC · DZL_AC · BRI_AC · NET_AC
- **수용체·신호전달 (5 구획)**: FP_RO · BETA_RO · ALPHA_RO · CA_INH · ROCK_INH
- **방수역학 (5 구획)**: ECM_CM · cAMP_CIL · AQ_PROD · C_TRAB · FU_UVEA
- **질환 (6 구획)**: IOP_cmt · ONH_STRESS · RGC_PCT · RNFL_UM · VF_MD · OPP_cmt

### 치료 시나리오 7가지

| # | 요법 | 주요 기전 | 근거 임상시험 |
|---|------|----------|--------------|
| S1 | 무치료 | Goldman 자연 경과 | — |
| S2 | 라타노프로스트 0.005% QD | FP→ECM→Fu↑ | EMGT, LiGHT |
| S3 | 티몰롤 0.5% BID | β2-AR→cAMP↓→F↓ | OHTS |
| S4 | 라타노프로스트 + 티몰롤 (Xalacom®) | 이중 기전 병합 | EGS guidelines |
| S5 | 3제 병용 (라타노+티몰롤+도르졸라미드) | 3가지 기전 | AGIS 보정 |
| S6 | 네타수딜 0.02% QD | ROCK→섬유주↑ | ROCKET-1/2 (Serle 2018) |
| S7 | 네타수딜 + 라타노프로스트 (Rocklatan®) | ROCK+FP 이중 | MERCURY-1/2 |

### 파일 목록

| 파일 | 설명 |
|------|------|
| [`glauc_qsp_model.dot`](glaucoma/glauc_qsp_model.dot) | Graphviz 기계론적 지도 (147+ 노드, 12 클러스터) |
| [`glauc_qsp_model.svg`](glaucoma/glauc_qsp_model.svg) | 벡터 형식 지도 (고해상도) |
| [`glauc_qsp_model.png`](glaucoma/glauc_qsp_model.png) | 래스터 형식 지도 (150 dpi) |
| [`glauc_mrgsolve_model.R`](glaucoma/glauc_mrgsolve_model.R) | mrgsolve ODE QSP 모델 (26구획, 7 시나리오) |
| [`glauc_shiny_app.R`](glaucoma/glauc_shiny_app.R) | Shiny 인터랙티브 대시보드 (8탭) |
| [`glauc_references.md`](glaucoma/glauc_references.md) | 참고문헌 62편+ (15개 섹션) |
| [`README.md`](glaucoma/README.md) | 디렉토리 상세 문서 |
