...
심방세동(AF)은 전 세계 3,300만 명 이상이 앓고 있는 가장 흔한 지속성 부정맥으로, 뇌졸중 위험 5배, 심부전 동반 시 5년 사망률 50%에 달합니다. 치료 전략은 **맥박수 조절(Rate Control)** vs **율동 조절(Rhythm Control)**과 **항응고 치료**의 조합으로 구성됩니다.

### 주요 병태생리 경로

| 경로 | 핵심 메커니즘 | 임상 이상 |
|------|------------|---------|
| 전기적 리모델링 | ICaL 하향조절 + IK1 상향조절 → AERP 단축 | "AF begets AF" — 자기영속화 |
| 폐정맥 이소성 박동 | PV sleeve 자동능 → 촉발활동 | AF 촉발 트리거 85–95% |
| 구조적 리모델링 | TGF-β1 → 섬유화 + Cx40/Cx43 이상 → 비균질 전도 | 좌심방 직경 확대, 만성 AF |
| 혈전색전증 | Virchow's triad (정체/내피손상/과응고) → LAA 혈전 | 뇌졸중 발생 |
| 신경호르몬 활성화 | RAAS + SNS → Ang II → 섬유화 가속 | 구조적 리모델링 가속 |
| 염증·산화 스트레스 | NLRP3 인플라마솜 + ROS → IL-6/TNF-α | 전기적·구조적 리모델링 가속 |

### 약물 PK/PD 파라미터

| 약물 | 분류 | 주요 PK | 기전 | 임상 효과 |
|------|------|---------|------|---------|
| 아미오다론 200mg/day | Class III | t½=40–55일, Vd=60L/kg | INa/ICaL/IKr/IKs/β 멀티차단 | 1년 동율동 유지 65% (AFFIRM) |
| 아픽사반 5mg BID | Factor Xa 억제제 | t½=12h, F=50%, CL=3.3L/h | 직접 FXa 억제 → 트롬빈 생성↓ | 뇌졸중 21%↓ vs 와파린 (ARISTOTLE) |
| 메토프롤롤 50mg BID | β1-blocker | t½=3–7h, F=40%, CL=65L/h | β1 차단 → AV 전도 억제 | 심박수 <110bpm (RACE II) |
| 리바록사반 20mg/day | Factor Xa 억제제 | t½=5–13h, F=80%(고지방식) | 직접 FXa 억제 | ROCKET AF: 비열등 vs 와파린 |
| 다비가트란 150mg BID | 직접 트롬빈 억제제 | t½=12–17h, P-gp/신장 배설 | 직접 IIa 억제 | RE-LY: 뇌졸중 34%↓ (150mg) |
| 드로네다론 400mg BID | Class III (비요오드) | t½=13–19h, F=15% | 멀티채널 차단 | ATHENA: CV 사망/입원 24%↓ |

### CHA₂DS₂-VASc 뇌졸중 위험 층화
...
| 파일 | 설명 |
|------|------|
| [af_qsp_model.dot](atrial-fibrillation/af_qsp_model.dot) | Graphviz 기계론적 지도 (329 노드, 12 클러스터) |
| [af_qsp_model.svg](atrial-fibrillation/af_qsp_model.svg) | SVG 벡터 이미지 |
| [af_qsp_model.png](atrial-fibrillation/af_qsp_model.png) | PNG 래스터 이미지 (150 dpi) |
| [af_mrgsolve_model.R](atrial-fibrillation/af_mrgsolve_model.R) | mrgsolve ODE 모델 (24 구획, 6 시나리오) |
| [af_shiny_app.R](atrial-fibrillation/af_shiny_app.R) | Shiny 대시보드 (6탭) |
| [af_references.md](atrial-fibrillation/af_references.md) | 참고문헌 46편 (PubMed 링크 포함) |

### 주요 치료 시나리오 (mrgsolve 시뮬레이션)

1. **미치료 신규 AF**: 2년간 AF 부담 증가, 섬유화 진행, CHA₂DS₂-VASc=4 → 뇌졸중 위험 4.0%/년
2. **맥박수 조절**: 메토프롤롤 50mg BID → 심박수 <110bpm, AF 부담 불변 (RACE II)
3. **율동 조절**: 아미오다론 200mg/day → 동율동 유지율 65%, AERP 정상화 (AFFIRM)
4. **항응고 단독**: 아픽사반 5mg BID → 뇌졸중 위험 21%↓ vs 와파린 (ARISTOTLE)
5. **맥박수 조절 + 항응고**: 메토프롤롤 + 아픽사반 → 심박수 조절 + 뇌졸중 예방
6. **율동 조절 + 항응고**: 아미오다론 + 아픽사반 → 동율동 유지 + 최대 뇌졸중 감소 (EAST-AFNET 4)

---

## 유방암 (Breast Cancer — ER+/HER2+/TNBC · CDK4/6 · HER2 · PD-1/PD-L1 · PARP)

### 개요

유방암은 전 세계적으로 여성에서 가장 흔한 악성 종양으로, 2020년 기준 연간 약 230만 명이 신규 진단되며 전체 암 발생의 11.7%를 차지합니다. 분자 아형에 따라 크게 세 가지로 분류됩니다: 호르몬 수용체 양성(HR+)/HER2 음성이 전체의 약 65~70%, HER2 양성(HER2+)이 15~20%, 삼중음성 유방암(TNBC)이 약 15%를 차지합니다. 각 아형은 독특한 발암 드라이버, 치료 감수성, 예후를 보이며, 맞춤형 치료 전략이 필수적입니다.

치료 패러다임은 아형별로 크게 다릅니다. ER+ 유방암에서는 CDK4/6 억제제(팔보시클립, 리보시클립, 아베마시클립)와 아로마타제 억제제의 병용이 1차 표준 치료로 자리 잡았으며, 무진행 생존 기간을 25~28개월로 연장하였습니다. HER2+ 유방암에서는 트라스투주맙·퍼투주맙 이중 차단 요법 및 차세대 항체-약물 접합체(T-DM1, T-DXd)가 현저한 생존 이득을 가져왔습니다. TNBC에서는 면역 관문 억제제(펨브롤리주맙), PARP 억제제(올라파립), 항체-약물 접합체(트라스투주맙 데룩스테칸)의 등장으로 치료 옵션이 급속히 확대되고 있습니다.

Breast cancer is the most common malignancy in women globally with ~2.3 million new cases annually. The QSP model developed here provides a mechanistic framework covering ER signaling, HER2 pathway, CDK4/6-RB cell cycle axis, tumor immune microenvironment, and resistance mechanisms, enabling simulation of treatment effects across 14 drug classes.

### 기계론적 지도

[![Breast Cancer QSP Model](breast-cancer/bc_qsp_model.png)](breast-cancer/bc_qsp_model.svg)

### 주요 병태생리 경로

| 경로 | 핵심 기전 | 임상 연관성 |
|------|---------|-----------|
| ER/ESR1 신호전달 | E2-ERα 결합 → ERE 전사 활성화 → Cyclin D1↑ → 세포증식 | 아로마타제 억제제·타목시펜·CDK4/6i 표적 |
| HER2/ERBB2 신호전달 | HER2 과발현 → PI3K/AKT/mTOR + MAPK/ERK 활성화 → 세포생존·증식 | 트라스투주맙·퍼투주맙·T-DM1/T-DXd 표적 |
| CDK4/6-Cyclin D1-RB 축 | CDK4/6-CyclinD1 복합체 → RB 인산화 → E2F 방출 → G1→S 진행 | 팔보시클립·리보시클립·아베마시클립 표적 |
| PI3K/AKT/mTOR | PIK3CA 활성화 돌연변이 (H1047R, E545K) → 세포생존 신호 | 알펠리십(PI3Kα 억제), 에베롤리무스(mTORC1 억제) |
| BRCA1/2-PARP 축 | BRCA 결함 → 상동재조합 불능 → 이중가닥 파손 축적 | 올라파립·탈라조파립 (PARP 합성 치사) |
| PD-1/PD-L1 면역 관문 | 종양 PD-L1 발현 → CD8+ T세포 기능 억제 | 펨브롤리주맙 (PD-L1 CPS≥10 TNBC) |
| 아로마타제 (CYP19A1) | 안드로스테네디온 → 에스트론 → 에스트라디올 변환 | 레트로졸·아나스트로졸·엑세메스탄 표적 |
| ESR1 돌연변이 내성 | Y537S, D538G 돌연변이 → 리간드 비의존성 ERα 활성화 | 아로마타제 억제제 2차 내성 기전 |

### 포함된 약물 및 임상시험

| 약물 | 분류 | 표적 | 대표 임상시험 | 효과 |
|------|------|------|------------|------|
| 팔보시클립 125mg PO QD | CDK4/6 억제제 | CDK4/CDK6 | PALOMA-2 | mPFS 27.6 mo (HR 0.56) |
| 리보시클립 600mg PO QD | CDK4/6 억제제 | CDK4/CDK6 | MONALEESA-2 | mPFS 27.5 mo (HR 0.57) |
| 아베마시클립 150mg PO BID | CDK4/6 억제제 | CDK4/CDK6 | MONARCH-3 | mPFS 28.2 mo |
| 레트로졸 2.5mg PO QD | 아로마타제 억제제 | CYP19A1 | PALOMA-2 | 병용 시 1차 표준 |
| 타목시펜 20mg PO QD | SERM | ERα | EBCTCG 메타분석 | 10년 재발 15% 감소 |
| 트라스투주맙 8mg/kg IV q3w | 항HER2 항체 | HER2/ERBB2 | CLEOPATRA | mPFS 18.7 mo (HR 0.62) |
| 퍼투주맙 420mg IV q3w | 항HER2 항체 | HER2 이합체화 도메인 | CLEOPATRA | 병용 mOS 56.5 mo |
| T-DM1 (트라스투주맙 엠탄신) | ADC | HER2 + 튜불린 | EMILIA | mPFS 9.6 mo (2차) |
| T-DXd (트라스투주맙 데룩스테칸) | ADC | HER2 + TopoI | DESTINY-Breast03 | mPFS 28.8 mo (HR 0.33) |
| 펨브롤리주맙 200mg IV q3w | 항PD-1 항체 | PD-1 | KEYNOTE-522 | EFS HR 0.63 (TNBC) |
| 올라파립 300mg PO BID | PARP 억제제 | PARP1/2 | OlympiAD | mPFS 7.0 mo (HR 0.58) |
| 알펠리십 300mg PO QD | PI3K 억제제 | PIK3Cα | SOLAR-1 | mPFS 11.0 mo (PIK3CA mut) |
| 에베롤리무스 10mg PO QD | mTOR 억제제 | mTORC1 | BOLERO-2 | mPFS 7.4 mo + 엑세메스탄 |
| 카페시타빈 | 전구약물 → 5-FU | 흉선딘 합성효소 | GEICAM/2003-11 | 전이성 구제 요법 |

### 모델 구조

| 구성요소 | 세부 사항 |
|--------|---------|
| 기계론적 지도 노드 | 110+ 노드, 9개 서브그래프 클러스터 |
| ODE 구획 | 22 구획 (PK 10개 + PD 12개) |
| 약물 PK | 팔보시클립 (2구획), 레트로졸 (1구획), 트라스투주맙 (2구획), 올라파립 (1구획), 펨브롤리주맙 (1구획) |
| 치료 시나리오 | 6개 (레트로졸 단독, 팔보+레트로졸, 트라스+퍼투+도세탁셀, 펨브롤리+화학요법, 올라파립, 무치료) |
| 시뮬레이션 기간 | 최대 730일 (2년) |
| 주요 경로 | ER/PI3K/AKT/mTOR, HER2/MAPK, CDK4/6-RB, DNA 손상/PARP, PD-1/PD-L1 |
| 바이오마커 | Ki-67, CA15-3, RB 인산화 억제율, CD8+ TIL, PD-L1, E2(혈장), ESR1 ctDNA |
| 내성 모듈 | ESR1 돌연변이, RB 소실, PI3K 활성화, CDK4/6i 우회(사이클린 E 증폭) |

### 모델 파일 목록

| 파일 | 설명 |
|------|------|
| [bc_qsp_model.dot](breast-cancer/bc_qsp_model.dot) | Graphviz 기계론적 지도 소스 (9 클러스터, 110+ 노드) |
| [bc_qsp_model.svg](breast-cancer/bc_qsp_model.svg) | SVG 벡터 이미지 (확대 가능) |
| [bc_qsp_model.png](breast-cancer/bc_qsp_model.png) | PNG 래스터 이미지 (150 dpi) |
| [bc_mrgsolve_model.R](breast-cancer/bc_mrgsolve_model.R) | mrgsolve ODE 모델 (22 구획, 6 시나리오, 5개 약물 PK) |
| [bc_shiny_app.R](breast-cancer/bc_shiny_app.R) | Shiny 대시보드 (6탭 인터랙티브 앱, 1424줄) |
| [bc_references.md](breast-cancer/bc_references.md) | 참고문헌 30+ 편 (PubMed 링크 포함, 9 섹션) |
| [README.md](breast-cancer/README.md) | 디렉토리 상세 설명 |

### 주요 치료 시나리오 (mrgsolve 시뮬레이션)

1. **레트로졸 단독 (ER+ 폐경 후 1차)**: 아로마타제 억제 → E2 >95% 감소 → ER 신호 차단 → 종양 성장 억제
2. **팔보시클립 + 레트로졸 (PALOMA-2, ER+/HER2-)**: CDK4/6 억제(RB 인산화 85% 차단) + ER 차단 → mPFS 27.6 mo
3. **트라스투주맙 + 퍼투주맙 + 도세탁셀 (CLEOPATRA, HER2+)**: HER2 이중 차단 → PI3K/MAPK 신호 억제 → 면역 효과기 활성화
4. **펨브롤리주맙 + 화학요법 (KEYNOTE-522, TNBC 신보조)**: PD-1 차단 → CD8+ T세포 활성화 + 면역 관문 해제 → pCR 64%
5. **올라파립 단독 (OlympiAD, BRCAm 전이성)**: PARP1/2 억제 → BRCA 결함 종양 합성 치사 → mPFS 7.0 mo
6. **무치료 (베이스라인)**: 자연 종양 성장 곡선 → 다른 시나리오의 비교 기준

### 핵심 참고문헌

1. Finn RS et al. (PALOMA-2). "Palbociclib and Letrozole in Advanced Breast Cancer." *N Engl J Med* 2016;375:1925–1936. [PMID 27959613](https://pubmed.ncbi.nlm.nih.gov/27959613/)
2. Hortobagyi GN et al. (MONALEESA-2). "Ribociclib as First-Line Therapy for HR+ Advanced Breast Cancer." *N Engl J Med* 2016;375:1738–1748. [PMID 27717303](https://pubmed.ncbi.nlm.nih.gov/27717303/)
3. Swain SM et al. (CLEOPATRA). "Pertuzumab, Trastuzumab, and Docetaxel in HER2-Positive Metastatic Breast Cancer." *N Engl J Med* 2015;372:724–734. [PMID 25693012](https://pubmed.ncbi.nlm.nih.gov/25693012/)
4. Schmid P et al. (KEYNOTE-522). "Pembrolizumab for Early Triple-Negative Breast Cancer." *N Engl J Med* 2020;382:810–821. [PMID 32101663](https://pubmed.ncbi.nlm.nih.gov/32101663/)
5. Robson M et al. (OlympiAD). "Olaparib for Metastatic Breast Cancer in Patients with a Germline BRCA Mutation." *N Engl J Med* 2017;377:523–533. [PMID 28578601](https://pubmed.ncbi.nlm.nih.gov/28578601/)
